#!/usr/bin/env python3
"""
Sec2Cass.py - Second Bar Data Importer for Cassandra Database

This script reads second bar data from stdin and inserts it into a Cassandra database.
The input data is expected to be in CSV or space-separated format with the following columns:
Date, Time, Open, High, Low, Close, Volume

Usage:
    python Sec2Cass.py <cassandra_ip> <cassandra_port> <database_name> <symbol>

Example:
    python Sec2Cass.py 192.168.1.217 9042 TQDB AAPL
...
def parse_date_time(date_str, time_str):
    ...
    try:
        ...
        # Parse time - note: for second bars, we might not convert float to int
        if ':' in time_str:
            time_parts = time_str.split(':')
            if len(time_parts) == 3:
                time_int = int(time_parts[0]) * 10000 + int(time_parts[1]) * 100 + int(time_parts[2])
        else:
            time_int = int(time_str)  # Different from Min2Cass - no float conversion
...
def loop_read_from_stdin():
    """
    Main processing loop that reads second bar data from stdin and inserts into Cassandra.
    ...
    line_count = 0
    print("Ready to insert second bars to database")
...
def main():
    ...
    if len(sys.argv) != 5:
        print("Usage: python Sec2Cass.py <cassandra_ip> <cassandra_port> <database_name> <symbol>")
        print("Example: python Sec2Cass.py 192.168.1.217 9042 TQDB AAPL")
        sys.exit(1)
