...
def prepare_import():
    ...
    with open(cmd_file, 'w', encoding='utf-8') as cmd:
        # Set environment variables directly (no profile script needed in container)
        cmd.write('#!/bin/bash\n')
        cmd.write('# Auto-generated import command for TQDB minute bar data\n\n')
        ...
        cmd.write(f'CASS_IP="{cassandra_host}"\n')
        cmd.write(f'CASS_PORT="{cassandra_port}"\n')
        cmd.write(f'TQDB="{cassandra_keyspace}"\n')
        cmd.write(f'TQDB_DIR="{tools_dir}"\n')
        ...
        cmd.write(f'CMD="cat /tmp/{import_ticket}.csv | '
                 f'python3 -u $TQDB_DIR/scripts/Min2Cass.py '
                 f'$CASS_IP $CASS_PORT $TQDB \'$SYMBOL\'"\n\n')
        
        # Add logging
        cmd.write(f'echo "Ready to run: $CMD" > /tmp/{import_ticket}.log\n')
        cmd.write(f'eval $CMD >> /tmp/{import_ticket}.log 2>&1\n')
...
def output_html_response(ret_obj):
    ...
    if param['Import'] == 'y':
        cmd = f'curl http://localhost/cgi-bin/i1min_do.py?importTicket={import_ticket}'
