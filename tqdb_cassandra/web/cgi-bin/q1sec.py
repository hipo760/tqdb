...
BIN_DIR = os.environ.get('TOOLS_DIR', '/opt/tqdb/tools/')  # TQ Database tools directory
...
def get_first_valid_datetime(symbol, begin_dt_str, end_dt_str):
    ...
    try:
        ...
        url = f'http://localhost/cgi-bin/qSymRefPrc.py?symbol={quote(symbol)}&qType=LastValidPrc&qDatetime={quote(begin_dt_str)}'
        ...
        url = f'http://localhost/cgi-bin/qSymRefPrc.py?symbol={quote(symbol)}&qType=LastValidPrc&qDatetime={quote(end_dt_str)}'
...
def download_from_tqdb(symbol, begin_dt, end_dt, tmp_file, gzip_enabled):
    """
    Download second-level bar data from TQ Database.
    
    Retrieves second-level OHLCV data directly from Cassandra using Python script.
    Queries the secbar table for the specified symbol and datetime range.
    
    Args:
        symbol (str): Trading symbol to query
        begin_dt (str): Start datetime in 'YYYY-MM-DD HH:MM:SS' format
    ...
    Process:
        1. Constructs Python command with Cassandra connection parameters
        2. Calls q1secall.py to query secbar table
        3. Script outputs CSV data to tmp_file (optionally gzipped)
        4. Returns number of records retrieved
        
    Output Format:
        CSV with columns: datetime,open,high,low,close,volume
        
    Environment Variables:
        - CASSANDRA_HOST: Cassandra server hostname/IP
        - CASSANDRA_PORT: Cassandra server port (default 9042)
    ...
    Note:
        Requires q1secall.py script in scripts/ directory
    """
    try:
