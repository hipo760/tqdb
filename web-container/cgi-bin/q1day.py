...
CASSANDRA_IP = os.environ.get('CASSANDRA_HOST', 'cassandra-node')  # Cassandra cluster IP address
...
def generate_daily_data(symbol, begin_dt, end_dt, tmp_file, gzip_enabled, mk_open, mk_close):
    """
    Generate daily OHLCV data by aggregating minute bars.
    
    Retrieves minute-level data from Cassandra and aggregates it into daily bars
    using Python script. Respects market session times for proper OHLC calculation.
    ...
    Process:
        1. Queries minbar table from Cassandra
        2. Filters minute bars by market session times
        3. Aggregates minute data into daily OHLCV bars
    ...
    Note:
        Requires q1dayall.py script in scripts/ directory
    """
    try:
