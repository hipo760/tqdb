...
def download_from_tqdb(symbol, begin_dt, end_dt, tmp_file, gzip_enabled):
    """
    Download minute-level data from TQ Database using Python script.
    
    Executes the q1minall.py Python script to extract minute-level bar data
    ...
    Process:
        1. Calls q1minall.py script with parameters
        2. Script queries Cassandra database for minute bar data
        3. Optionally aggregates ticks into 1-minute OHLCV bars for gaps
        4. Outputs to temporary file (optionally gzipped)
        
    Note:
        Requires q1minall.py script in scripts directory with execute permissions
    """
    try:
        ...
        cmd = f"python3 {scripts_dir}/q1minall.py {cassandra_host} {cassandra_port} {cassandra_keyspace} '{symbol}' '{begin_dt}' '{end_dt}' '{tmp_file}' '{gzip_enabled}'"
